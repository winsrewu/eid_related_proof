...
/-! # Group clustering — the stage-0 converse spawn-origin fact

At the pop tick of stage `0`, the two reference observer events `A` and
`D` (both priority `0`) spawn the stage-`1` events `sA` and `sD`. This
file proves the stage-`0` analogue of the converse spawn-origin fact of
ConverseSpawn: any event between `sA` and `sD` in the spawn accumulator is the
stage-`1` event of some chain, and that chain's stage-`0` observer sits
between `A` and `D` in the due filter.

Unlike the middle-stage converse (`converse_spawn_popSpawnAcc`, which
needs `1 ≤ j` and a `MiddleBlockOk` premise), the stage-`0` case is
unconditional: the priority squeeze between two priority-`0` pops forces
the parent to priority `0`, and `stagePri = 0` holds only at stage `0`,
so the parent is a stage-`0` observer. No middle-block invariant is
required. This supplies the `j = 0 → j = 1` step that the unified step of
GSimBodyIteration cannot cover. -/
...
/-- At the pop tick of stage `0`, two reference observer events `A` and
    `D` spawn `sA` and `sD`. An event between `sA` and `sD` in the spawn
    accumulator is the stage-`1` event of some chain, and that chain's
    stage-`0` observer sits between `A` and `D` in the due filter. -/
theorem converse_spawn_popSpawnAcc_stage0 (groups : List GroupSpec)
    (actTick : Nat → Nat) (w : World) (g₁ c₁ g₂ c₂ n : Nat)
  ...
  -- basic facts about the two reference observer events
  ...
  have h_AeR : evBefore (World.popSeqFuel w n) A eR := by
    rwa [← h_parent_eq]
  -- the parent sits between two priority-0 pops, so it has priority 0
  have h_pri_eR : eR.priority = (0 : Int) := by
    ...
    rw [hD_pri] at h_le₂
    omega
  -- priority 0 occurs only at stage 0, so the parent is a stage-0 observer
  have h_k_zero : k = 0 := by
    have h_p : stagePri groups gi ci k = (0 : Int) := by
    ...
    split_ifs at h_p
    all_goals omega
  -- the parent is the stage-0 observer of chain (gi, ci)
  have h_eR0 : eR = stageEvent actTick groups gi ci 0 := by
    rw [h_eR_eq, h_k_zero]
  ...
  refine ⟨gi, ci, h_gi, h_ci, ?_, ?_, ?_⟩
  · -- e is the stage-1 event of chain (gi, ci)
    rw [h_e_eq, h_k_zero]
  · -- the stage-0 observer of (gi, ci) sits after A in the due filter
    rwa [← h_eR0]
  · -- the stage-0 observer of (gi, ci) sits before D in the due filter
    rwa [← h_eR0]
...
/-- At the pop tick of stage `0`, two reference observer events spawn
    `sA` and `sD` during the drain step. An event between `sA` and `sD`
    in the post-drain queue `w.stepUntilNextTick.events` is the stage-`1`
    event of some chain, and that chain's stage-`0` observer sits between
    the two reference observers in the due filter. Wraps
    `converse_spawn_popSpawnAcc_stage0` by splitting the post-drain queue
    into survivors and the spawn accumulator. -/
theorem converse_spawn_stepUNT_stage0 (groups : List GroupSpec)
    (actTick : Nat → Nat) (w : World) (g₁ c₁ g₂ c₂ : Nat)
    (h_g₁ : g₁ < groups.length) (h_c₁ : c₁ < (groupAt groups g₁).length)
    (h_g₂ : g₂ < groups.length) (h_c₂ : c₂ < (groupAt groups g₂).length)
    (h_layout : NodeLayoutOk groups w)
    (h_due : w.tick = stageTarget actTick groups g₁ c₁ 0)
  ...
  have h_b_right : evBefore (World.popSpawnAcc w n) e sD := by
    ...
    · exact h_b
  exact converse_spawn_popSpawnAcc_stage0 groups actTick w g₁ c₁ g₂ c₂ n
    h_g₁ h_c₁ h_g₂ h_c₂ h_layout h_due h_tgt₂ h_nodup h_stage
    h_sA_absent h_sD_absent e h_e_absent h_b_left h_b_right
